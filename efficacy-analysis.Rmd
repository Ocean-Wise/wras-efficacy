...
Therefore the aims of the project are to... \* Use survey data to understand more about mariners behavior when using the WRAS from their perspective \* Use AIS and WRAS alert data to test how often a vessel changes speed or direction due to a WRAS alert
...
The final steps before we have cleaned alert data ready to process the AIS data - \* we need to filter for users from Ocean Wise and other non-mariner organizations who should not be included in the efficacy analysis - they would likely not take action to a WRAS alert. \* create a time interval of before and after an alert was sent to compare during analysis. This will also be used to reduce the size of the AIS data via a join later on.
...
The AIS data is already in a clean format, but we need to process it. Processing steps: \* filter to remove ships which are: at anchor; moored; not under control; restricted maneuverability; pushing or towing alongside, or engaged in fishing. + This [link](https://datalastic.com/blog/ais-navigational-status/) describes the catagories of *status* well. \* Then need to join data if the MSSI matches, the date matches, a the time is -30 or +30 mins from the time of the alert.
...
baseline_loop_function = function(file_path) {
  baseline_data = data.table::fread(file_path) %>% 
    ...
    dplyr::mutate(local_time = lubridate::force_tz(lubridate::as_datetime(local_time), tzone = "Canada/Pacific"),
                  date = lubridate::date(local_time)) %>% 
    dplyr::select(c("date", "mmsi", "name", "type", "sog", "cog", "heading"))
...
leaflet::leaflet(data = low_speed_map) %>% 
  leaflet::addTiles() %>% 
  leaflet::addCircleMarkers(lng = ~longitude.x, lat = ~latitude.x,
                            popup = ~(paste(name, type)))  
  

```

Filtering the speed down to \<0.2 knots shows that ships at these speeds are broadly always in or very close to port. Although not outliars in the study, these ships won't slow down due to being slowed down already + in port so I think I should remove them from the analysis.

Filter out all of the data under 0.2 knots + over 40.2 knots. Group by class and calculate difference to median for speed of vessels. The only issue with filtering out data under 0.2 knots and 40.2 knots might give me a skewed amount of data points before and after the alert was sent, although, looking at the AIS data it seems like that was already the case...
...
-   We need to filter the data by vessels \> 0.2 knots and \< 40 knots
...
median_value_filtered = clean_wras_ais_filtered %>%
  dplyr::filter(type %in% vessel_type_filter) %>%
  dplyr::group_by(type) %>%
  dplyr::summarise(median_test = median(sog))

median_value_non_filtered = clean_wras_ais_non_filtered %>%
  dplyr::filter(type %in% vessel_type_filter) %>%
  dplyr::group_by(type) %>%
  dplyr::summarise(median_test = median(sog))


wras_filtered = clean_wras_ais_filtered %>% 
  dplyr::filter(type %in% vessel_type_filter) %>%
  dplyr::filter(sog > 7) %>% 
  dplyr::left_join(median_baseline_filtered, by = "type") %>% 
  dplyr::left_join(median_value_filtered, by = "type") %>% 
  dplyr::group_by(type) %>%  
  ggplot2::ggplot(ggplot2::aes(x = sog)) +
  ggplot2::geom_histogram(binwidth = 1) +
  ggplot2::geom_vline(ggplot2::aes(xintercept = median), color = "red", linetype = "dashed") +
...
wras_non_filtered = clean_wras_ais_non_filtered %>% 
  dplyr::filter(type %in% vessel_type_filter) %>%
  dplyr::left_join(median_baseline_non_filtered, by = "type") %>% 
  dplyr::left_join(median_value_non_filtered, by = "type") %>% 
  dplyr::group_by(type) %>%  
  ggplot2::ggplot(ggplot2::aes(x = sog)) +
  ggplot2::geom_histogram(binwidth = 1) +
  ggplot2::geom_vline(ggplot2::aes(xintercept = median), color = "red", linetype = "dashed") +
...
# Create histograms for each group with adjusted axes
ggpubr::ggarrange(wras_filtered, wras_non_filtered,
                  labels = c("filtered", "non-filtered"),
                  nrow = 2)
