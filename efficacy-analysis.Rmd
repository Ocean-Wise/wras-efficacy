...
baseline_vs_wras = filtered_baseline_data %>% 
  dplyr::filter(type %in% median_filtered_wras_data$type) %>% 
...
-   To dos...
    ...
    -   [ ] Clean the cargo ship data to remove outliers and smaller vessels than the min of the test group.

    -   [ ] Clean passenger category - smaller vessels to be removed (based on scatter plot)
...
x = median_filtered_baseline_data %>% 
  dplyr::filter(type == "Passenger ship", sog >= 23.5) 
